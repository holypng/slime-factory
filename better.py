...
class Slime:
    def __init__(self, name, traits, base_value):
        ...
        if "legendary" in self.traits:
            ...
            self.hp += 8
    def get_value(self):
        return self.base_value * len(self.traits) * self.size
    def __str__(self):
        trait_str = ""
...
while True:
    ...
    if choice == "1":
        ...
        if len(inventory.slimes) < 2:
            ...
            elif ("metallic" in slime1.traits and "magnetic" in slime2.traits) or ("magnetic" in slime1.traits and "metallic" in slime2.traits):     
                new_slime = Slime("steel slime", ["crafted"], (slime1.get_value() + slime2.get_value()) / 2)
            elif ("sparkling" in slime1.traits and "charged" in slime2.traits) or ("charged" in slime1.traits and "sparkling" in slime2.traits):     
                new_slime = Slime("electric slime", ["crafted"], (slime1.get_value() + slime2.get_value()) / 2)
            else:
                new_slime = craft_slimes(slime1, slime2)
            if new_slime is not None:
                inventory.slimes.append(new_slime)
                inventory.slimes.remove(slime1)
                inventory.slimes.remove(slime2)
